...
- **`customer.createPlanChangeSession(params)`** — the self-service half: a customer switches one of their own subscriptions to another plan, using their session token. This is the call `selfServicePlanChange` actually gates. The platform applies three checks here that merchant-issued links never face, each a 403: the subscription is the session customer's, the target plan is in the **same product group**, and that group's switch is on.
- **`CustomerPlanChangeParams`** — deliberately narrower than the merchant params. A customer-session request carries no merchant id, so the platform silently drops every API-Key-only field; rather than accept and ignore them, the type omits `changeAmount`, `changeCreditAmount`, `withTrial`, `priceSnapshot`, `expiresInSeconds`, `metadata`, `orderMerchantExternalId`, `includePaymentMethods` and `excludePaymentMethods`. What remains: `originOrderId`, `productId`, `currency`, `changeTiming`, `successUrl`, `darkMode`, `language`.
- **`GroupRules.selfServicePlanChange`** — the group-level master switch for customers changing plans within the group from the customer portal. While it is off, `customer.createPlanChangeSession()` is rejected with a 403; merchant-issued links are unaffected. Writable through `subscriptionProductGroups.create()` / `update()`, readable back on the group entity.

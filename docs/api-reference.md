...
| `paymentMethods`          | `PaymentMethod[]`        | No       | Ordered whitelist restricting which payment methods the hosted checkout page shows. Must be non-empty, without duplicates, and actually available for this session's currency/environment/product type — otherwise `create-session` returns a 4xx error. Omit to keep current default behavior. |
...
| `language`                | `string`                 | No       | Default checkout cashier language (IETF BCP 47), e.g. `pt-BR`, `zh-Hant-TW`. Must be one of the supported cashier languages; the customer can switch on the page.                                                                                                                 |
| `paymentMethods`          | `PaymentMethod[]`        | No       | Ordered whitelist restricting which payment methods the hosted checkout page shows. Honored on the API Key path; visitor / store-slug flows silently drop it. Must be non-empty, without duplicates, and actually available for this session's currency/environment/product type. |
...
| `language`                | `string`                 | No       | Default checkout cashier language (IETF BCP 47), e.g. `pt-BR`, `zh-Hant-TW`. Must be one of the supported cashier languages; the customer can switch on the page.                                                                                                  |
| `paymentMethods`          | `PaymentMethod[]`        | No       | Ordered whitelist restricting which payment methods the hosted checkout page shows. Honored on the API Key (merchant) path; visitor / store-slug flows silently drop it. Must be non-empty, without duplicates, and actually available for this session's context. |
